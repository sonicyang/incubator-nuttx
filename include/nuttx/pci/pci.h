...
inline int pci_cfg_write(FAR struct pci_dev_s *dev, uintptr_t addr,
...
inline uint32_t pci_cfg_read(FAR struct pci_dev_s *dev, uintptr_t addr,
                             unsigned int size)
{
  DEBUGASSERT(size == 1 || size == 2 || size == 4);
...
inline int pci_msix_register(FAR struct pci_dev_s *dev,
                             uint32_t vector, uint32_t index)
{
...
inline int pci_msi_register(FAR struct pci_dev_s *dev, uint16_t vector)
{
